...
mkdir -p model
mkdir -p ./model/code2review_trans_data_task2/cache/
mkdir -p ./model/code2review_trans_data_task2/outputs/
mkdir -p ./model/code2review_trans_data_task2/summary/
mkdir -p ./model/code2review_trans_data_task2/outputs/results

# Đường dẫn tới checkpoint gốc (read-only trên Kaggle)
CKPT_SRC=/kaggle/input/checkpoint-0/checkpoint-epoch-0/checkpoint.pt
# Thư mục outputs của train Trans (writable)
OUT=./model/code2review_trans_data_task2/outputs

# Copy checkpoint cũ vào đúng vị trí để script resume
mkdir -p ${OUT}/checkpoint-epoch-0
cp ${CKPT_SRC} ${OUT}/checkpoint-epoch-0/

# Chạy script huấn luyện Trans Data
