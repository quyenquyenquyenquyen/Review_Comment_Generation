...
CKPT_SRC=/kaggle/input/checkpoint-10/checkpoint-epoch-10
...
mkdir -p ${OUT}/checkpoint-epoch-10
cp ${CKPT_SRC}/checkpoint.pt ${OUT}/checkpoint-epoch-10/

# Chạy script huấn luyện
CUDA_VISIBLE_DEVICES=0 python run_gen.py \
    --do_train --do_eval --do_eval_bleu \
    --task refine --sub_task small --model_type codet5 \
    --data_num -1 --num_train_epochs 11 --warmup_steps 500 \
